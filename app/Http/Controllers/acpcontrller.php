...
class acpcontrller extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $KGID = session()->get("KGID");


        $district = DB::select("SELECT  District_Name FROM fir_data WHERE KGID='$KGID'");
        ...
        $DySP = DB::select("SELECT COUNT(IOName) AS count FROM fir_data  WHERE IOName LIKE '%(Dy.Sp)%' AND District_Name = '$districtname'")[0]->count;
        ...
        return view(
            'acpdash',
            compact(
                'district','UnitName','VICTIM_COUNT','Accused_Count','Conviction_Count','Accused_ChargeSheeted_Count','RI',
                'Complaint_Mode','Beat_Name','Village_Area_Name','yearlyCounts','DySP','PI','PSI','ASI','HConstable','Constable',
                'Male','Female','MonthFIRCounts','MonthHeinousCounts','MonthNonHeinousCounts','ArrestedCounts','ArrestedMaleCounts','ArrestecFemaleCounts'
            )
        );
    }
    ...
    public function show(string $id)
    {
        ...
        $district = DB::select("SELECT DISTINCT District_Name FROM fir_data WHERE KGID='$KGID'");
        $UnitName = DB::select("SELECT DISTINCT UnitName FROM fir_data WHERE District_Name = '$districtname'" );
        $VICTIM_COUNT = DB::select("SELECT SUM(VICTIM_COUNT) AS count FROM fir_data WHERE VICTIM_COUNT != 0 AND District_Name = '$districtname' ")[0]->count;
        $Accused_Count = DB::select("SELECT SUM(Accused_Count) AS count FROM fir_data WHERE Accused_Count != 0 AND District_Name = '$districtname' ")[0]->count;
        $Conviction_Count = DB::select("SELECT SUM(Conviction_Count) AS count FROM fir_data WHERE Conviction_Count != 0 AND District_Name = '$districtname' ")[0]->count;
        $Accused_ChargeSheeted_Count = DB::select("SELECT SUM(Accused_ChargeSheeted_Count) AS count FROM fir_data WHERE Accused_ChargeSheeted_Count != 0 AND District_Name = '$id' ")[0]->count;
        $RI = DB::select("SELECT SUM(RI) AS count FROM fir_data WHERE RI != 0 AND District_Name = '$districtname'")[0]->count;
        $Complaint_Mode = DB::select("SELECT COUNT(Complaint_Mode) AS count FROM fir_data Where  District_Name = '$districtname' ")[0]->count;
        $Beat_Name = DB::select("SELECT COUNT(DISTINCT Beat_Name) AS count FROM fir_data WHERE  District_Name = '$districtname' ")[0]->count;
        $Village_Area_Name = DB::select("SELECT COUNT(DISTINCT Village_Area_Name) AS count FROM fir_data Where  District_Name = '$districtname' ")[0]->count;
