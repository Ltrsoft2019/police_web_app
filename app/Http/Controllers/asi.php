...
class asi extends Controller
{
    ...
    public function index()
    {
        ...
        $PI = 0;//DB::select("SELECT COUNT(IOName) AS count FROM fir_data  WHERE IOName LIKE '%(PI)%' AND District_Name = '$districtname'")[0]->count;
        $PSI =0;// DB::select("SELECT COUNT(IOName) AS count FROM fir_data  WHERE IOName LIKE '%(PSI)%' AND District_Name = '$districtname'")[0]->count;
        $ASI = 0;//DB::select("SELECT COUNT(IOName) AS count FROM fir_data  WHERE IOName LIKE '%(ASI)%' AND District_Name = '$districtname'")[0]->count;
        ...
        return view('asidash',compact('district','UnitName','VICTIM_COUNT','Accused_Count','Conviction_Count','Accused_ChargeSheeted_Count','RI','Complaint_Mode',"Beat_Name",'Village_Area_Name','yearlyCounts'
